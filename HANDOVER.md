...
## Engine atlas, 13 September 2026

While the book waits for the Astra read, Barry asked for a full mathematical
map of the engine to guide any further tuning. `docs/engine_atlas.md` is the
result, with figures and tables regenerated by `tools/analysis/atlas.py`
(`uv sync --extra analysis` for matplotlib). Ranked findings: Twilight's
combat stances are unbalanced (Vanguard dominates, Watchful never correct);
Advantage on attack is worth about twice Advantage on defence and the Almanac
does not say so; plate equals mail against anything but edge +2; Luck is more
sustainable than the book's caution implies; advancement plateaus after four
milestones by design; NPC design lacks party-size guidance; one crisis per
